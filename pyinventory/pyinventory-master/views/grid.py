...
class ReadOnlyTable(Screen):
    def __init__(self, dataList, title):
        ...
        table_layout.add_widget(
            Button(text="Show stats", size_hint_y=None, height=50, font_size=16, on_press=self.stats))

        self.show_tip_btn = Button(text="Tips", size_hint_y=None, size_hint=(0.2, None), height=50,
                                       font_size=16,
    ...
    def stats(self, event):
        sales_data = {"sales": 0, "nonsales": 0, "service": 0}
        quantity_data = {"sales": 0, "nonsales": 0, "service": 0}
        """
        Categories : 
        CONDITIONER
        SHAMPOO
        ...
        # print(self.dataList)
        for d in self.dataList:
            if "sales" in d or "SALES" in d:
                sales_data["sales"] = float(sales_data["sales"]) + float(d[3])
                quantity_data["sales"] = float(quantity_data["sales"]) + float(d[4])
            elif "nonsales" in d or "NONSALES" in d:
                sales_data["nonsales"] = float(sales_data["nonsales"]) + float(d[3])
                quantity_data["nonsales"] = float(quantity_data["nonsales"]) + float(d[4])
        ...
        import numpy as np
        commodities = ['Sales ({})'.format(int(quantity_data["sales"])),
                       'NonSales ({})'.format(int(quantity_data["nonsales"])),
